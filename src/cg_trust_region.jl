...
CGTrustRegion(; initial_radius::Real = 1.0,
                    max_radius::Real = 100.0,
                    eta::Real = 0.1,
                    rho_lower::Real = 0.25,
                    rho_upper::Real = 0.75,
                    cg_tol::Real = 0.01) =
                    CGTrustRegion(initial_radius, max_radius, eta,
                                  rho_lower, rho_upper, cg_tol)


type CGTrustRegionState{T,N,G}
    @add_generic_fields()
    g::G
    f_x_previous::T
    x_previous::Vector{T}
...
function initial_state{T}(method::CGTrustRegion, options, d, initial_x::Array{T})
    n = length(initial_x)
    ...
    CGTrustRegionState("Conjugate Gradient Newton's Method (Trust Region)",
                         length(initial_x),
...
function cg_steihaug!{T}(objective::TwiceDifferentiableHV,
                         state::CGTrustRegionState{T},
                         method::CGTrustRegion)
    n = length(state.x)
