...
SourceBrightness{NumType <: Number}(vs::Vector{NumType}) = begin
    ...
    for i = 1:Ia
        ...
        E_l_a[4, i].v = exp(c1[3, i] + .5 * c2[3, i]) * E_l_a[3, i].v

        # Band 5 = band 4 * color 4.
        E_l_a[5, i].v = exp(c1[4, i] + .5 * c2[4, i]) * E_l_a[3, i].v

        # Band 2 = band 3 * color 2.
        E_l_a[2, i].v = exp(-c1[2, i] + .5 * c2[2, i]) * E_l_a[3, i].v

        # Band 1 = band 2 * color 1.
        E_l_a[1, i].v = exp(-c1[1, i] + .5 * c2[1, i]) * E_l_a[2, i].v
