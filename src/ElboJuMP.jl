...
function build_jump_model(blob::Blob, mp::ModelParams)
    ...
    source_pixels = Array(Vector{Tuple{Int64, Int64}}, mp.S);
    for s = 1:mp.S
        source_pixels[s] = Array(Tuple{Int64, Int64}, 0)
    end
    ...
    @variable(m, 0  <= vp_chi[s=1:mp.S] <= 1)

     # The location of the object.
    @variable(m, vp_mu[s=1:mp.S, axis=1:2])

    # Ix1 scalar variational parameters for r_s.  The first
    # row is for stars, and the second for galaxies (I think?).
    @variable(m, vp_gamma[s=1:mp.S, i=1:CelesteTypes.I] >= 0)
    @variable(m, vp_zeta[s=1:mp.S,  i=1:CelesteTypes.I] >= 0)

    # The weight given to a galaxy of type 1.
    @variable(m, 0 <= vp_theta[s=1:mp.S] <= 1)

    # galaxy minor/major ratio
    @variable(m, 0 <= vp_rho[s=1:mp.S] <= 1)

    # galaxy angle
    # TODO: bounds?
    @variable(m, vp_phi[s=1:mp.S])

    # galaxy scale
    @variable(m, vp_sigma[s=1:mp.S] >= 0)
    ...
    @variable(m, 0 <= vp_kappa[s=1:mp.S, d=1:CelesteTypes.D, a=1:CelesteTypes.I] <= 1)

    # (B - 1)xI matrices containing c_s means and variances, respectively.
    @variable(m, vp_beta[s=1:mp.S,   b=1:(CelesteTypes.B - 1), a=1:CelesteTypes.I])
    ...
    @NLexpression(m, E_l_a_3[s=1:mp.S, a=1:CelesteTypes.I],
               vp_gamma[s, a] * vp_zeta[s, a]);

    # The remaining indices involve c_s and have lognormal
    # expectations times E_c_3.
    @NLexpression(m, E_l_a_4[s=1:mp.S, a=1:CelesteTypes.I],
               E_l_a_3[s, a] * exp(vp_beta[s, 3, a] + .5 * vp_lambda[s, 3, a]));
    @NLexpression(m, E_l_a_5[s=1:mp.S, a=1:CelesteTypes.I],
               E_l_a_4[s, a] * exp(vp_beta[s, 4, a] + .5 * vp_lambda[s, 4, a]));

    @NLexpression(m, E_l_a_2[s=1:mp.S, a=1:CelesteTypes.I],
               E_l_a_3[s, a] * exp(-vp_beta[s, 2, a] + .5 * vp_lambda[s, 2, a]));
    @NLexpression(m, E_l_a_1[s=1:mp.S, a=1:CelesteTypes.I],
    ...
    @NLexpression(m, E_l_a[s=1:mp.S, b=1:CelesteTypes.B, a=1:CelesteTypes.I],
               sum{E_l_a_1[s, a]; b == 1} +
               sum{E_l_a_2[s, a]; b == 2} +
               sum{E_l_a_3[s, a]; b == 3} +
               sum{E_l_a_4[s, a]; b == 4} +
    ...
    # Second order terms.
    @NLexpression(m, E_ll_a_3[s=1:mp.S, a=1:CelesteTypes.I],
               vp_gamma[s, a] * (1 + vp_gamma[s, a]) * vp_zeta[s, a] ^ 2);

    @NLexpression(m, E_ll_a_4[s=1:mp.S, a=1:CelesteTypes.I],
               E_ll_a_3[s, a] * exp(2 * vp_beta[s, 3, a] + 2 * vp_lambda[s, 3, a]));
    @NLexpression(m, E_ll_a_5[s=1:mp.S, a=1:CelesteTypes.I],
               E_ll_a_4[s, a] * exp(2 * vp_beta[s, 4, a] + 2 * vp_lambda[s, 4, a]));

    @NLexpression(m, E_ll_a_2[s=1:mp.S, a=1:CelesteTypes.I],
               E_ll_a_3[s, a] * exp(-2 * vp_beta[s, 2, a] + 2 * vp_lambda[s, 2, a]));
    @NLexpression(m, E_ll_a_1[s=1:mp.S, a=1:CelesteTypes.I],
               E_ll_a_2[s, a] * exp(-2 * vp_beta[s, 1, a] + 2 * vp_lambda[s, 1, a]));

    # TODO: make the indexing consistent with the rest of the file by
    # putting the band first.
    @NLexpression(m, E_ll_a[s=1:mp.S, b=1:CelesteTypes.B, a=1:CelesteTypes.I],
    ...
    @NLexpression(m, star_mean[b=1:CelesteTypes.B, s=1:mp.S, k=1:n_pcf_comp, row=1:2],
    ...
    # This is W' * W
    @NLexpression(m, galaxy_xixi_mat[s=1:mp.S, row=1:2, col=1:2],
               vp_sigma[s]^2*
    ...
    @NLexpression(m, galaxy_type1_var_s[b=1:CelesteTypes.B, s=1:mp.S,
    ...
    @NLexpression(m, galaxy_type1_det[b=1:CelesteTypes.B, s=1:mp.S,
                                k=1:n_pcf_comp, g_k=1:n_gal1_comp],
               (galaxy_type1_var_s[b, s, k, g_k, 1, 1] *
                galaxy_type1_var_s[b, s, k, g_k, 2, 2]) -
               (galaxy_type1_var_s[b, s, k, g_k, 1, 2]^2))

    @NLexpression(m, galaxy_type2_det[b=1:CelesteTypes.B, s=1:mp.S,
                                k=1:n_pcf_comp, g_k=1:n_gal2_comp],
    ...
    @NLexpression(m, galaxy_type1_precision[b=1:CelesteTypes.B, s=1:mp.S,
                                      k=1:n_pcf_comp, g_k=1:n_gal1_comp,
    ...
    @NLexpression(m, galaxy_type2_precision[b=1:CelesteTypes.B, s=1:mp.S,
                                      k=1:n_pcf_comp, g_k=1:n_gal2_comp,
                                      prec_row=1:2, prec_col=1:2],
    ...
    @NLexpression(m, galaxy_type1_z[b=1:CelesteTypes.B, s=1:mp.S,
                              k=1:n_pcf_comp, g_k=1:n_gal1_comp],
                (galaxy_type1_alpha_tilde[g_k] * psf_alpha_bar[b, k]) /
                (sqrt(galaxy_type1_det[b, s, k, g_k]) * 2pi));

    @NLexpression(m, galaxy_type2_z[b=1:CelesteTypes.B, s=1:mp.S,
                              k=1:n_pcf_comp, g_k=1:n_gal2_comp],
    ...
    @NLexpression(m, star_pdf_mean[img=1:CelesteTypes.B, s=1:mp.S, k=1:n_pcf_comp,
    ...
    @NLexpression(m, star_pdf_f[img=1:CelesteTypes.B, s=1:mp.S, k=1:n_pcf_comp,
    ...
    # Star density values:
    @NLexpression(m, fs0m[img=1:CelesteTypes.B, s=1:mp.S, pw=1:img_w, ph=1:img_h],
               sum{star_pdf_f[img, s, k, pw, ph], k=1:n_pcf_comp});

    # Galaxy density values:
    @NLexpression(m, fs1m[img=1:CelesteTypes.B, s=1:mp.S, pw=1:img_w, ph=1:img_h],
    ...
    @NLexpression(m, E_G_s[img=1:CelesteTypes.B, s=1:mp.S, pw=1:img_w, ph=1:img_h],
               (1 - vp_chi[s]) * E_l_a[s, img, 1] * fs0m[img, s, pw, ph] +
               vp_chi[s]       * E_l_a[s, img, 2] * fs1m[img, s, pw, ph]);

    @NLexpression(m, Var_G_s[img=1:CelesteTypes.B, s=1:mp.S, pw=1:img_w, ph=1:img_h],
    ...
    @NLexpression(m, E_G[img=1:CelesteTypes.B, pw=1:img_w, ph=1:img_h],
               sum{E_G_s[img, s, pw, ph], s=pixel_sources[pw, ph]} + blob_epsilon[img]);

    @NLexpression(m, Var_G[img=1:CelesteTypes.B, pw=1:img_w, ph=1:img_h],
    ...
    @NLexpression(m, pixel_log_likelihood[img=1:CelesteTypes.B, pw=1:img_w, ph=1:img_h],
    ...
    @NLexpression(m, img_log_likelihood[img=1:CelesteTypes.B],
               sum{pixel_log_likelihood[img, pw, ph],
                   pw=1:img_w, ph=1:img_h});

    @NLexpression(m, elbo_log_likelihood,
